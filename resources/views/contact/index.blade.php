...
<section class="content-header">
    <h1 class="tw-text-xl md:tw-text-3xl tw-font-bold tw-text-black"> @lang('lang_v1.' . $type . 's')
        <small
            class="tw-text-sm md:tw-text-base tw-text-gray-700 tw-font-semibold">@lang('contact.manage_your_contact', ['contacts' => __('lang_v1.' . $type . 's')])</small>
    </h1>
</section>
    ...
    @if (!empty($api_key))
        ...
        <script type="text/javascript">
            $(document).on('shown.bs.modal', '.contact_modal', function (e) {
                initAutocomplete();
                if ($(this).find('select#contact_state').val() === '' || $(this).find('select#contact_state').val() === null) {
                    $(this).find('select#contact_country').trigger('change');
                }
            });

            // Dynamic dropdowns for country, state, city
            $(document).on('change', 'select#contact_country', function () {
                var country = $(this).val();
                var modal = $(this).closest('.modal');
                var state_dropdown = modal.find('select#contact_state');
                var city_dropdown = modal.find('select#contact_city');
                ...
                if (country) {
                    $.ajax({
                        ...
                        success: function (result) {
                            ...
                            // Trigger change on state if it has a value (e.g. pre-selected in Blade)
            ...
            $(document).on('change', 'select#contact_state', function () {
                var state = $(this).val();
                var modal = $(this).closest('.modal');
                var city_dropdown = modal.find('select#contact_city');
