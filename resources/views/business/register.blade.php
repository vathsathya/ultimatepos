        ...
        <div class="tw-flex tw-flex-col tw-gap-4 tw-dw-rounded-box tw-dw-p-6 tw-dw-max-w-md">
            <div class="tw-flex tw-flex-col rounded-2xl tw-dw-p-6 tw-dw-max-w-md text-center">
                {{-- <svg xmlns="http://www.w3.org/2000/svg"
                    class="tw-ml-4 tw-mt-4 icon icon-tabler icon-tabler-circle-key-filled" width="44" height="44"
                ...
                {{-- <svg xmlns="http://www.w3.org/2000/svg"
                    class="tw-mr-3 tw-mt-1 icon icon-tabler icon-tabler-lock-up" width="44" height="44"
                    viewBox="0 0 24 24" stroke-width="1.5" stroke="#2c3e50" fill="none" stroke-linecap="round"
                    stroke-linejoin="round">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                ...
                <h1 class="tw-dw-text-3xl tw-dw-font-bold"> {{ config('app.name', 'ultimatePOS') }} -
                    @lang('business.register_and_get_started_in_minutes')</h1> --}}
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#country').change(function () {
                ...
                if (country) {
                    $.ajax({
                        url: "{{ route('business.getStates') }}",
                        method: 'GET',
                        data: { country: country },
                        dataType: 'json',
                        success: function (result) {
                            $('#state').empty().append('<option value="">' + "{{ __('business.state') }}" + '</option>');
                            $('#city').empty().append('<option value="">' + "{{ __('business.city') }}" + '</option>');
