...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            // Telegram — root-level menu item (visible once module is installed)
            if (\Module::has('Telegram') && \App\System::getProperty('telegram_version')) {
                if (auth()->user()->can('business_settings.access')) {
                    $menu->url(
                        route('telegram-settings'),
                        __('telegram::lang.telegram'),
                        [
                            'icon' => '<svg xmlns="http://www.w3.org/2000/svg" class="tw-size-5 tw-shrink-0" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.894 8.221-1.97 9.28c-.145.658-.537.818-1.084.508l-3-2.21-1.447 1.394c-.16.16-.295.295-.605.295l.213-3.053 5.56-5.023c.242-.213-.054-.333-.373-.12l-6.871 4.326-2.962-.924c-.643-.204-.657-.643.136-.953l11.57-4.461c.537-.194 1.006.131.833.941z"/></svg>',
                            'active' => request()->segment(1) == 'telegram-settings',
                        ]
                    )->order(82);
                }
            }
