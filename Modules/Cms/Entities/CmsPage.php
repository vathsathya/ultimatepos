...
class CmsPage extends Model
{
    ...
    public static function getEnabledPages($type = 'page', $page_position = 'page_dropdown')
    {
        $pages = CmsPage::where('type', $type)
                    ->whereNull('layout')
                    ->where(function($query) use ($page_position) {
                        if($page_position == 'page_dropdown'){
                            $query->whereNull('page_position')->orWhere('page_position', 'page_dropdown');
                        } elseif($page_position == 'seperate_menu'){
                            $query->where('page_position', 'seperate_menu');
                        }
                    })
