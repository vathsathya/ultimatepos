            ...
            <div class="container-fluid">
                ...
                <div id="navbar" class="navbar-collapse collapse">
                    <ul class="nav navbar-nav">
                        ...
                        <li><a href="#" class="menu" id="share_catalogue" title="Share Catalogue"><i class="fas fa-share-alt"></i> Share</a></li>
                    </ul>
                    <form class="navbar-form navbar-right">
                        <div class="form-group">
                            <input type="text" id="search_product" class="form-control search-input"
                    ...
                    @foreach($product_category as $product)
                        <div class="col-md-3 eq-height-col col-xs-12">
                            <div class="box box-solid product-box">
                                <div class="box-body" style="padding: 0; padding-bottom: 15px;">
                                    <a href="#" class="show-product-details"
                                        ...
                                        <img src="{{$product->image_url}}" class="img-responsive catalogue" loading="lazy"></a>
                                    ...
                                    @endphp
                                    <h2 class="catalogue-title text-center">
                                        <a href="#" class="show-product-details"
                                            data-href="{{action('\Modules\ProductCatalogue\Http\Controllers\ProductCatalogueController@show',  [$business->id, $product->id])}}?location_id={{$business_location->id}}">
                                            {{$product->name}}
                                        </a>
                                    ...
                                    <table class="table no-border product-info-table"
                                        style="margin-bottom: 0px; padding: 0 10px;">
                                        <tr>
                                            ...
                                            <td class="pb-0">
                                                ...
                                                data-currency_symbol="true">{{($min_price)}}</span> @endif
                                            </td>
                                        </tr>
                                        <tr>
                                                        ...
                                                        <select class="form-control input-sm">
                                                            @foreach($product_variation as $variation)
                                                                <option value="{{$variation->id}}">{{$variation->name}}
                                                                    ({{$variation->sub_sku}}) - {{($variation->sell_price_inc_tax)}}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    </td>
...
@section('javascript')
    <script type="text/javascript">
        ...
        (function ($) {
            ...
            $('#search_product').keyup(function () {
                ...
                $('.product-category-wrap').each(function () {
                    ...
                    $(this).find('.product-box').each(function () {
                        var title = $(this).find('.catalogue-title').text().toLowerCase();
                        var sku = $(this).find('tr:nth-child(2) td').text().toLowerCase();
                        if (title.indexOf(search) > -1 || sku.indexOf(search) > -1) {
                            $(this).parent().show();
            ...
            $(document).on('click', '#share_catalogue', function(e){
                e.preventDefault();
                var url = window.location.href;
                if (navigator.share) {
                    navigator.share({
                        title: '{{$business->name}} Product Catalogue',
