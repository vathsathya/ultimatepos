...
class SubscriptionController extends BaseController
{
    ...
    public function pay(Request $request, $package_id, $form_register = null)
    {
        ...
        try {
            ...
            if (env('BAKONG_ACCOUNT_ID')) {
                try {
                    $merchantInfo = new \KHQR\Models\MerchantInfo(
                        env('BAKONG_ACCOUNT_ID'), // bakongAccountID
                        env('APP_NAME', 'UltimatePOS'), // merchantName
                        'Phnom Penh', // merchantCity
                        'POS_MERCHANT', // merchantID
                        'ACLEDA' // acquiringBank
                    );
                    $merchantInfo->amount = (float) ($package_price_after_discount > 0 ? $package_price_after_discount : $package->price);
                    $merchantInfo->currency = \KHQR\Helpers\KHQRData::CURRENCY_USD; // Assuming USD for now

                    $khqrResp = \KHQR\BakongKHQR::generateMerchant($merchantInfo);
                    $khqr_string = $khqrResp->data['qr'] ?? null;
                    if (!$khqr_string && is_array($khqrResp->data) && isset($khqrResp->data['qr'])) {
                        $khqr_string = $khqrResp->data['qr'];
                    }
                } catch (\Exception $e) {
            ...
            return view('superadmin::subscription.pay')
                ->with(compact('package', 'gateways', 'system_currency', 'layout', 'user', 'offline_payment_details', 'coupon_status', 'package_price_after_discount', 'discount_amount', 'khqr_string'));
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    /**
     * Bakong KHQR check status using MD5
     */
    public function bakongCheck(Request $request)
    {
        $md5 = $request->input('md5');
        $bakong_token = env('BAKONG_TOKEN');

        if (!$md5 || !$bakong_token) {
            return response()->json(['success' => false, 'msg' => 'Missing MD5 or Bakong Token']);
        }

        try {
            $isTest = config('app.env') !== 'production';
            $response = \KHQR\Api\Transaction::checkTransactionByMD5($bakong_token, $md5, $isTest);

            if (isset($response['responseCode']) && $response['responseCode'] == 0) {
                return response()->json(['success' => true, 'msg' => 'Paid successfully', 'data' => $response]);
            }

            return response()->json(['success' => false, 'msg' => 'Payment pending or failed', 'data' => $response]);
        } catch (\Exception $e) {
            \Log::error('Bakong verification failed: ' . $e->getMessage());
            return response()->json(['success' => false, 'msg' => 'Error checking transaction: ' . $e->getMessage()]);
        }
    }
