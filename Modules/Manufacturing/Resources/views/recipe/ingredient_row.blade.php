<tr>
	<td>
		...
		<input type="hidden" name="ingredients[{{$row_index}}][ingredient_id]" class="ingredient_id"
			value="{{$ingredient->id}}">
		...
		@if(!empty($ingredient->ingredient_line_id))
			<input type="hidden" name="ingredients[{{$row_index}}][ingredient_line_id]"
				value="{{$ingredient->ingredient_line_id}}">
		@endif

		@if(!empty($ingredient->mfg_ingredient_group_id))
			<input type="hidden" name="ingredients[{{$row_index}}][mfg_ingredient_group_id]"
					...
					<select name="ingredients[{{$row_index}}][sub_unit_id]" class="form-control input-sm row_sub_unit_id">
						@foreach($ingredient->sub_units as $key => $value)
							<option value="{{$key}}" data-multiplier="{{$value['multiplier']}}"
								@if(!empty($ingredient->sub_unit_id) && $key == $ingredient->sub_unit_id) selected @endif>
								{{$value['name']}}
							</option>
						@endforeach
					</select>
	...
	<td><button type="button" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline  tw-dw-btn-error remove_ingredient"><i
